...
export default function App() {
  const [currentUser, setCurrentUser] = useState(() => {
    try { return localStorage.getItem("daftar_user") || null; } catch { return null; }
  });
  ...
  useEffect(() => {
    ...
    try {
      unsub = fbListen(
        (data) => {
          const normalized = data.map((l) => ({ ...l, category: normalizeCategory(l.category) }));
  ...
  function handleLogin(user) {
    setCurrentUser(user);
    try { localStorage.setItem("daftar_user", user); } catch {}
  }

  function handleLogout() {
    setCurrentUser(null);
    try { localStorage.removeItem("daftar_user"); } catch {}
  }
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative" }}>
      ...
      <Header currentUser={currentUser} onLogout={handleLogout} stats={stats} onOpenReview={() => setShowReview(true)} />
